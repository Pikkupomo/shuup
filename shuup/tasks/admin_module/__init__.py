# -*- coding: utf-8 -*-
# This file is part of Shuup.
#
# Copyright (c) 2012-2018, Shoop Commerce Ltd. All rights reserved.
#
# This source code is licensed under the OSL-3.0 license found in the
# LICENSE file in the root directory of this source tree.
from __future__ import unicode_literals

from django.utils.translation import ugettext_lazy as _

from shuup.admin.base import AdminModule, MenuEntry
from shuup.admin.menu import CONTENT_MENU_CATEGORY
from shuup.admin.utils.permissions import get_default_model_permissions
from shuup.admin.utils.urls import derive_model_url, get_edit_and_list_urls
from shuup.admin.views.home import HelpBlockCategory, SimpleHelpBlock
from shuup.simple_cms.models import Page
from shuup.tasks.models import Task


class TaskAdminModule(AdminModule):
    name = _(u"Shuup Tasks")
    breadcrumbs_menu_entry = MenuEntry(name, "shuup_admin:tasks.list")

    def get_urls(self):
        return get_edit_and_list_urls(
            url_prefix="^tasks/",
            view_template="shuup.tasks.admin_module.views.Task%sView",
            name_template="tasks.%s",
            permissions=get_default_model_permissions(Task)
        )

    def get_menu_entries(self, request):
        return [
            MenuEntry(
                text=_("Tasks"), icon="fa fa-file-text",
                url="shuup_admin:tasks.list",
                category=CONTENT_MENU_CATEGORY,
                subcategory="elements",
                ordering=3,
                aliases=[_("Show Tasks")]
            )
        ]

    # def get_help_blocks(self, request, kind):
    #     if kind == "quicklink":
    #         yield SimpleHelpBlock(
    #             text=_("Add a web page"),
    #             actions=[{
    #                 "text": _("New page"),
    #                 "url": self.get_model_url(Page, "new")
    #             }],
    #             priority=100,
    #             category=HelpBlockCategory.STOREFRONT,
    #             icon_url="simple_cms/page.png"
    #         )

    def get_required_permissions(self):
        return get_default_model_permissions(Task)

    def get_model_url(self, object, kind, shop=None):
        return derive_model_url(Page, "shuup_admin:tasks", object, kind)
